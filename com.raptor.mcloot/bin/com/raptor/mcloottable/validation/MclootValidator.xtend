...
//            }
    ...
    def checkLootFunction(EnchantRandomlyLootFunction it) {
        if (!enchantments.isEmpty) {
            checkUniqueValues(ENCHANT_RANDOMLY_LOOT_FUNCTION__ENCHANTMENTS, [String enchantment, i |
                ...
                return addDefaultNamespace(enchantment)
    ...
    def checkLootCondition(ToolEnchantmentLootCondition it) {
        if (enchantments.isEmpty) {
            ...
            checkUnique(enchantments, [addDefaultNamespace(enchantment)], ENCHANTMENT_LEVEL__ENCHANTMENT, "Duplicate enchantment level entry")
        }
    }
    
    @Check
    def checkEnchantmentLevel(EnchantmentLevel it) {
        ifMinecraftKey(enchantment) [enchantment |
            if (!isVanillaEnchantment(enchantment)) {
                warning("Unknown vanilla enchantment name",
    ...
    def checkEntityPredicate(EntityPredicate it) {
        if (effectsDefined) {
            if (effects.isEmpty) {
                ...
                checkUnique(effects, [addDefaultNamespace(id)], EFFECT_PREDICATE__ID, "Duplicate effect predicate")
            }
        }
        ...
        if (type !== null) {
            ifMinecraftKey(type) [type |
                if (!isVanillaEntityType(type)) {
                    warning("Unknown vanilla entity type",
                        ENTITY_PREDICATE__TYPE
                    )
                }
                if (type === it.type) {
                    info("Namespace defaulting to 'minecraft'",
    ...
    @Check
    def checkEffectPredicate(EffectPredicate it) {
        ifMinecraftKey(id) [id |
